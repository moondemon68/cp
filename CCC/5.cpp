...
int main () {
    ...
    cout.tie(NULL);
    for (LL t=2;t<=2;t++) {
        ifstream cin("level5/level5_" + to_string(t) + ".in");
        ofstream cout("level5/level5_" + to_string(t) + ".out");
        ...
        for (LL i=1;i<=m;i++) {
            ...
            while (x) {
                ...
                // cout << ans.fi << " ";
                if (ans.fi == INT_MAX) cerr << i << " " << ans.se << endl;
                answer.pb({ans.se, k});
                left[ans.se+1] -= k;
                con[ans.se+1]--;
                // cout << ans.se << " " << k << " ";
            ...
            for (auto tp : v) {
                if (left[tp.se+1] > 0 && con[tp.se+1] > 0) updateRange(1,1,n,tp.se+1,tp.se+1,-INT_MAX);
            }
            sort (answer.begin(), answer.end());
            for (auto tp : answer) {
                cout << tp.fi << " " << tp.se << " ";
                if (tp.fi < l || tp.fi > r) cerr << t << " " << i << endl;
            }
            cout << '\n';
