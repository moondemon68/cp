...
int main () {
    ...
    cout.tie(NULL);
    for (LL t=0;t<=5;t++) {
        ifstream cin("level6/level6_" + to_string(t) + ".in");
        ofstream cout("level6/level6_" + to_string(t) + ".out");
        ...
        for (int i=1;i<=n;i++) con[i] = maxCon;
        build(1,1,n);
        LL m;
        cin >> m;
        cout << m << '\n';
        for (LL i=1;i<=m;i++) {
            ...
            while (x) {
                ...
                v.pb(ans);
                LL k = min(maxPower-left[ans.se+1], x);
                x -= k;
                updateRange(1,1,n,ans.se+1,ans.se+1,INT_MAX);
                // cout << ans.fi << " ";
                // if (ans.fi == INT_MAX) cerr << "FORCED MAX: " << i << " " << ans.se << endl;
                answer.pb({ans.se, k});
                left[ans.se+1] += k;
                con[ans.se+1]--;
                // cout << ans.se << " " << k << " ";
            ...
            for (auto tp : v) {
                if (left[tp.se+1] <= maxPower && con[tp.se+1] > 0) updateRange(1,1,n,tp.se+1,tp.se+1,-INT_MAX);
            }
            sort (answer.begin(), answer.end());
            for (auto tp : answer) {
                cout << tp.fi << " " << tp.se << " ";
                if (tp.fi < l || tp.fi > r) cerr << "INVALID RANGE: " << t << " " << i << endl;
            }
            cout << '\n';
        ...
        double bill = 0;
        for (int i=1;i<=n;i++) {
            bill += (double)a[i].fi * ((double)1 + (double)(left[i]/maxPower));
        }
        cerr << bill << " " << maxBill << endl;
